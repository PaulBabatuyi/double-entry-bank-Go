...
const dashboard = {
  ...
  /**
   * Fast refresh path after a successful mutation.
   * Updates balances first, then refreshes transaction history in the background.
   */
  async refreshAfterMutation() {
    await this.loadAccounts({ loadTransactions: false });
    this.updateStats();
    this.scheduleTransactionRefresh();
  },

  /**
   * Refresh transactions immediately and once more shortly after,
   * to absorb platform/network latency without blocking the UI.
   */
  scheduleTransactionRefresh() {
    void this.loadLatestTransactions();
    setTimeout(() => {
      void this.loadLatestTransactions();
    }, 1200);
  },
   ...
   * Load user accounts
   */
  async loadAccounts(options = { loadTransactions: true }) {
    try {
      ...
      if (response.ok) {
        ...
        if (options.loadTransactions !== false) {
